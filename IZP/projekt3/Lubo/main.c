...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
  ...
  // TODO
  float result = -1, currentDistance;
...
/**
 * checks if the first line of text contains only string "count=" and integer number
...
int load_clusters(char *filename, struct cluster_t **arr)
{
  ...
  {
    fprintf(stderr, "Number of objects can't be less than %d", objectsCount);
    clearClusters(*arr, objectsCount);
    *arr = NULL;
    fclose(file);
    return -1;
...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1 && argc <= 3)
  {
    ...
    int newSize;
    if ((newSize = setClustersCount(clusters, size, finalSize)) == -1)  //if function returns -1, exit program
    {
