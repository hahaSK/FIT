...
*/
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2, const int calc_mode)
{
  assert(c1 != NULL);
  assert(c1->size > 0);
  assert(c2 != NULL);
...
/*
 ...
 adresu 'c1' resp. 'c2'.
*/
void find_neighbours(struct cluster_t *carr, int narr, int *c1, int *c2, const int calc_mode)
{
  assert(narr > 0);
  ...
  for (int i=0; i<narr;i++)
  {
    for (int j=i+1; j<narr; j++)
    {
      ...
      curr_dist=cluster_distance(&carr[i],&carr[j], calc_mode);
...
int set_clusters_count(struct cluster_t *carr, int narr, const int finalSize, const int calc_mode)
{
  if (finalSize > narr)
  {
  ...
  {
    find_neighbours(carr, narr, &idx1, &idx2, calc_mode);
