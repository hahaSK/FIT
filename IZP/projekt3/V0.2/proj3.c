...
#define AVG 1   //Calculates average distance
...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
  ...
  if (premium_case == AVG)
    result = curr_dist / (c2->size * c1->size);
 ...
 * @param narr - size of cluster array
 */
...
/**
 * checks if objects count is correct, initialize clusters and checks allocation
 * @param file - file of objects.
 * @param load - array of one line.
 * @param arr - pointer to loaded clusters from file.
 * @return negative number on error, otherwise returns objects count
 */
...
int get_count(FILE *file, char *load, struct cluster_t **arr)
{
  ...
  {
    fprintf(stderr, "sscanf function error !\n");
    fclose(file);
    return -1;
  }
...
/**
 * checks file of obejcts for errors
 * @param input - value (number from file)
 * @param index - index of integer in line (max 3 integers in line)
 * @param line - number of line
 * @return negative number on error, otherwise returns value (number from file)
 */
...
int check_values(int input, int index, int line)
{
  if ((index == 0) && (input < 0))
  {
    fprintf(stderr, "IDs on line %d in text file is negative (lower than 0).\n", line);
    return -7;
  }
  if (((index == 1) || (index == 2)) && ((input < 0) || (input > MAXIMALVALUEOFCOORDINATES)))
  {
    fprintf(stderr, "On line %d was found coordinate, that is out of the defined interval (0,1000)\n", line);
    return -8;
  }
  return input;
...
/**
 * saves numbers from file to object
 * @param arr - pointer to loaded clusters from file.
 * @param loaded_values - loaded values from file
 * @param curr_obj - current object
 */

void save_input_lines(struct cluster_t **arr, int *loaded_values, int curr_obj)
{
  struct obj_t obj;
  struct cluster_t *cluster;

  obj.id = loaded_values[0];
  obj.x = loaded_values[1];
  obj.y = loaded_values[2];
  cluster = &(*arr)[curr_obj];
...
/**
 * checks for additional errors in file
 * @param file - file of objects
 * @param arr - pointer to loaded clusters from file.
 * @param load - loaded values from file
 * @param curr_obj - current object
 * @param obj_count - objects count
 * @param ids - array of IDs
 * @return false if character is not valid
 */

bool check_input_lines(FILE *file, struct cluster_t **arr, char *load, int curr_obj, int obj_count, int *ids)
{
  char last_char;
  bool not_valid_char = false;
  int loaded_values[ARGINLINE];

  if (sscanf(load, "%d %d %d%[^\n]", &loaded_values[0], &loaded_values[1], &loaded_values[2], &last_char) !=
      ARGINLINE)
  {
    fprintf(stderr, "line %d has invalid format. Should contain 3 numbers of integer type separated by ' '\n",
            curr_obj + 2);
    not_valid_char = true;
  }
  for (int i = 0; i < ARGINLINE; ++i)
  {
    loaded_values[i] = check_values(loaded_values[i], i, curr_obj + 2);
    if (loaded_values[i] < 0)
      not_valid_char = true;
  }

  for (int i = 0; i < curr_obj; i++)
  {
    if (loaded_values[0] == ids[i])
    {
  ...
  ids[curr_obj] = loaded_values[0];

  if (not_valid_char == 1)
  {
    clear_input_clusters(*arr, obj_count);
    *arr = NULL;
    fclose(file);
  }
  else
    save_input_lines(arr, loaded_values, curr_obj);
...
int load_clusters(char *filename, struct cluster_t **arr)
{
  ...
  while (current_object < stream_char_count)
  {
    ...
    if (check_input_lines(file, arr, load, current_object, stream_char_count, ids) == true)
      return -9;
...
int input_validation(char *argv)
{
  ...
  if ((strcmp(argv, "--avg") == 0) || (strcmp(argv, "--min") == 0) || (strcmp(argv, "--max") == 0))
    return -11;
  else if (*endchar != '\0')
    return -1;
  else if (result <= 0)
...
/**
 * chcecks if the cluster count is correct
 * @param carr - cluster array
 * @param narr - size of cluster array
 * @param end_size - final size of cluster
 * @return size of cluster array
 */

int set_clusters_count(struct cluster_t *carr, int narr, const int end_size)
{
  if (end_size > narr)
  {
    fprintf(stderr, "Value of cluster count, which is '%d' can't be bigger than objects count '%d'.", end_size, narr);
    return -5;
  }
...
void err_report(int err_code)
{
  switch (err_code)
  {
    ...
    case (-4):
      ...
      break;
    case (-6):
      fprintf(stderr, "get_count function Error !\n");
      break;
    case (-9):
      fprintf(stderr, "Error in objects file !\n");
      break;
    case (-11):
      fprintf(stderr, "Argument N is required when using mode parameter !\n");
      break;
...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1 && argc <= 4)
  {
    ...
    if (argc == 3 || argc == 4)
    {
      end_size = input_validation(argv[2]);
      if (end_size < 0)
      {
        err_report(end_size);
        return EXIT_FAILURE;
    ...
    if ((size = load_clusters(argv[1], &clusters)) < 0)
    {
      err_report(size);
      return EXIT_FAILURE;
    }
    ...
    int resize;
    if ((resize = set_clusters_count(clusters, size, end_size)) < 0)
    {
