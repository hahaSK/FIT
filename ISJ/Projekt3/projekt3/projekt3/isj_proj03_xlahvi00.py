...
def match_permutations(string, words):
    ...
    matching_perms = sorted(word for word in words if word in perms)  # vase reseni
...
def plur2sing(singular, plural):
    ...
    # inversion using zip
    pl2sg = {p: s for p, s in zip(plural, singular)} # vase reseni
    return pl2sg

def vect2word(word2vect):
    v2w = dict((tuple(value), key) for (key, value) in word2vect.items())
    return v2w
